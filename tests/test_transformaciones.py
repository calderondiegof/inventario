...
def test_informe_material_clasificacion() -> None:
    """Informe por material: ``obtener_informe_material`` consulta
    ``movimientos_inventario`` y agrupa entradas por fuente y salidas por
    observación. Para Revuelto, la salida por transformación se etiqueta
    como ``'Selección'``.

    Prueba unitaria: datos simulados, sin depender de BD.
    """
    from services.inventario_service import InventarioServiceConValidacion, TipoTransaccion

    def _mov(*, tipo: TipoTransaccion, material: str, cantidad_kg: float,
              fecha: str, bodega_id: int = 1, fuente: Optional[str] = None,
              observaciones: Optional[str] = None) -> Dict[str, Any]:
        return {
            "bodega_id": bodega_id, "material_id": 1,
            "tipo_movimiento": tipo.value, "cantidad_kg": cantidad_kg,
            "fecha_operacion": fecha, "lote_operacion_id": "lot",
            "observaciones": observaciones,
            "fuentes_origen": ({},) if fuente is None else ({"nombre": fuente},),
            "materiales": ({"nombre": material},),
        }

    # Simular movimientos previos para saldo inicial.
    def _simular_cache_movimientos(filas: List[Dict[str, Any]], material: str) -> None:
        """Inserta filas simuladas en la cache de consulta para test unitario."""
        InventarioServiceConValidacion._simular_movimiento_en_filas_cache(filas, 1, material)

    def _limpiar_cache() -> None:
        """Vacía la cache de movimientos después del test."""
        InventarioServiceConValidacion._limpiar_cache_movimientos()

    # --- Material NO-revuelto (Cobre): compra entra, remisión sale ---
    filas_cobre_compra = [_mov(tipo=TipoTransaccion.COMPRA, material="Cobre",
                                cantidad_kg=500.0, fecha="2026-09-10",
                                fuente="Compras")]
    filas_cobre_remision = [_mov(tipo=TipoTransaccion.VENTA, material="Cobre",
                                  cantidad_kg=-150.0, fecha="2026-09-10",
                                  observaciones="Remisión")]
    filas_cobre = filas_cobre_compra + filas_cobre_remision
    _simular_cache_movimientos(filas_cobre, "Cobre")

    resultado_cobre = InventarioServiceConValidacion.obtener_informe_material(
        bodega_id=1, material_nombre="Cobre",
        fecha_desde="2026-09-10", fecha_hasta="2026-09-10",
    )
    entradas_map_c = {e["etiqueta"]: e["kg"] for e in resultado_cobre["entradas"]}
    salidas_map_c = {s["etiqueta"]: s["kg"] for s in resultado_cobre["salidas"]}

    assert abs(entradas_map_c.get("Compras", 0) - 500) < 0.01, "Cobre: compra debe entrar 500"
    assert abs(salidas_map_c.get("Remisión", 0) - 150) < 0.01, "Cobre: remisión debe salir 150"

    # --- Material REVUELTO: Cooperativa y Pesca entran; selección sale ---
    filas_revuelto_entrada = [
        _mov(tipo=TipoTransaccion.ENTRADA_BRUTA, material="Revuelto",
             cantidad_kg=10.0, fecha="2026-09-10", fuente="Cooperativa"),
        _mov(tipo=TipoTransaccion.ENTRADA_BRUTA, material="Revuelto",
             cantidad_kg=3.0, fecha="2026-09-10", fuente="Pesca"),
    ]
    filas_revuelto_salida = [
        _mov(tipo=TipoTransaccion.TRANSFORMACION, material="Revuelto",
             cantidad_kg=-15.0, fecha="2026-09-10",
             observaciones="Selección"),
        # Remisión con 0 kg: debe descartarse (no aparecer).
        _mov(tipo=TipoTransaccion.VENTA, material="Revuelto",
             cantidad_kg=0.0, fecha="2026-09-10",
             observaciones="Remisión"),
    ]
    filas_revuelto = filas_revuelto_entrada + filas_revuelto_salida
    _simular_cache_movimientos(filas_revuelto, "Revuelto")

    resultado_revuelto = InventarioServiceConValidacion.obtener_informe_material(
        bodega_id=1, material_nombre="Revuelto",
        fecha_desde="2026-09-10", fecha_hasta="2026-09-10",
    )
    entradas_map_r = {e["etiqueta"]: e["kg"] for e in resultado_revuelto["entradas"]}
    salidas_map_r = {s["etiqueta"]: s["kg"] for s in resultado_revuelto["salidas"]}

    assert abs(entradas_map_r.get("Cooperativa", 0) - 10) < 0.01, "Revuelto: Cooperativa entra 10"
    assert abs(entradas_map_r.get("Pesca", 0) - 3) < 0.01, "Revuelto: Pesca entra 3"
    assert abs(salidas_map_r.get("Selección", 0) - 15) < 0.01, "Revuelto: selección sale 15"
    assert salidas_map_r.get("Remisión/Venta", 0) == 0, "Revuelto: remisión cero no debe aparecer"
    assert abs(sum(e["kg"] for e in resultado_revuelto["entradas"]) - 13) < 0.01, "Revuelto: total entradas = 13"
    # El saldo inicial en prueba unitaria es 0 (no hay movimientos previos insertados).
    assert resultado_revuelto["saldo_inicial"] == 0.0, "Saldo inicial debe ser 0 en prueba unitaria"
    assert abs(resultado_revuelto["total_entradas"] - 13) < 0.01, "Total entradas debe ser 13"
    assert abs(resultado_revuelto["total_salidas"] - 15) < 0.01, "Total salidas debe ser 15"
    assert abs(resultado_revuelto["movimiento"] + 2) < 0.01, "Movimiento debe ser -2"  # 13 - 15 = -2
    assert abs(resultado_revuelto["saldo_final"] + 2) < 0.01, "Saldo final debe ser -2"

    # Limpieza: vaciar la cache.
    _limpiar_cache()
