"""Servicio de inventario con validación de datos.

Este módulo orquesta el ciclo de vida de las operaciones de inventario
sobre Supabase: registra entradas, compras, ventas, transformaciones,
selección de revuelto y aprobaciones. La lógica de bajo nivel (parsing,
normalización, formato de WhatsApp) vive en módulos ``utils/`` y los
enumerados/DTOs en ``services/enums.py`` y ``services/types.py``.

Compatibilidad: los símbolos públicos que se exportaban desde aquí siguen
disponibles vía re-imports para no romper consumidores externos
(handlers, main, tests).
"""
import difflib
import hashlib
...
# ---------------------------------------------------------------------------
# Re-imports para preservar el contrato público (compatibilidad hacia atrás).
# ---------------------------------------------------------------------------
# Enums
from services.enums import TipoMaterial, TipoTransaccion  # noqa: F401
# DTOs
from services.types import MaterialDTO, FuenteDTO  # noqa: F401
...
# Parseo de números / moneda
from utils.number_parser import (
    _parsear_numero,
    _parsear_numero_moneda,
)  # noqa: F401
# Utilidades de WhatsApp (parsing/formatos de listas y precios)
from utils.whatsapp_utils import (
    construir_lista_texto_whatsapp,
    resolver_entrada_material,
    construir_seccion_lista_interactiva,
    formatear_resumen_precios,
    parsear_edicion_precio,
    procesar_precio_paso_a_paso,
    es_lista_materiales,
    borrador_para_nueva_lista,
)  # noqa: F401
# Parsing de bloques y sinónimos / deduplicación
...
# ---------------------------------------------------------------------------
# NOTA DE MIGRACIÓN: Las definiciones de las funciones originales que vivían
# en este módulo (construcción de listas de WhatsApp, parseo de números,
# parsing de bloques de persona, sinónimos y frases, deduplicación por huella)
# fueron extraídas a módulos ``utils/`` y ``services/enums.py|types.py``.
# Se re-exportan arriba para preservar el contrato público.
# ---------------------------------------------------------------------------
...
class InventarioServiceConValidacion:
    ...
    def __init__(self, supabase_client: Client):
        ...
        self.conductor_service = ConductorService(supabase_client)
    ...
    def obtener_conductor_por_nombre(self, nombre: str) -> Optional[Dict[str, Any]]:
        """Busca un conductor por nombre en la tabla 'conductores' (delegado a conductor_service)."""
        return self.conductor_service.obtener_conductor_por_nombre(nombre)

    def obtener_conductor_por_id(self, conductor_id: Any) -> Optional[Dict[str, Any]]:
        """Busca un conductor por su ID (delegado a conductor_service)."""
        return self.conductor_service.obtener_conductor_por_id(conductor_id)
    ...
    def obtener_o_crear_conductor(self, *, nombre: str, identificacion: Optional[str] = None,
        ...
        return self.conductor_service.obtener_o_crear_conductor(
    ...
    def buscar_conductor_existente(self, *, identificacion: Optional[str] = None,
                                    ...
                                    nombre: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Solo búsqueda (sin crear): delega en conductor_service."""
        return self.conductor_service.buscar_conductor_existente(
            identificacion=identificacion, placa=placa, nombre=nombre
        )
    ...
    def registrar_conductor(self, *, nombre_conductor: str,
        ...
        """Crea un conductor en la tabla 'conductores' (delegado a conductor_service)."""
        return self.conductor_service.registrar_conductor(
            nombre_conductor=nombre_conductor,
            id_conductor=id_conductor,
            telefono_conductor=telefono_conductor,
            direccion_conductor=direccion_conductor,
            placa_conductor=placa_conductor,
            placa_trailer_conductor=placa_trailer_conductor,
        )
