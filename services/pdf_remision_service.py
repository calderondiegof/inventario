"""Servicio de PDFs de remision: busqueda, descarga y envio WhatsApp."""
...
class PdfRemisionService:
    ...
    async def enviar_pdf_remision(self, telefono: str, numero_remision: str, *, enviar_documento_fn=None) -> Tuple[bool, str]:
        remision = self.obtener_pdf_remision(numero_remision)
        if not remision:
            return False, (f"No encontre el PDF para {chr(39)}{numero_remision}{chr(39)}. Verifica el numero e intenta de nuevo.")
        try:
        ...
        try:
            await enviar_documento_fn(destino=telefono, ruta_archivo=str(ruta), nombre_documento=remision.filename)
        except Exception as exc:
...
def construir_filas_listado_pdf(remisiones):
    filas = []
    for r in remisiones[:10]:
        ...
        fecha = ""
        if r.fecha_creacion:
            try: fecha = datetime.fromisoformat(str(r.fecha_creacion).replace("Z","")).strftime("%Y-%m-%d")
            except: fecha = str(r.fecha_creacion)[:10]
        filas.append((num, num[:24], f"{cliente[:40]}{chr(45)+chr(32)+fecha if fecha else chr(32)}"))
    return filas
